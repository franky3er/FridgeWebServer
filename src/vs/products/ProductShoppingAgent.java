...
public class ProductShoppingAgent {
    ...
    private void refillIfNeeded(ScannedProduct product, ProductRefillInfo productRefillInfo) {
        ...
        Map.Entry<String, ShopService.Client> cheapestShop = getCheapestShop(product.getName(), refillAmount);
        buyProduct(product.getName(), refillAmount, cheapestShop);
    }
    ...
    public boolean orderProduct(String productName, String productAmount) {
        Map.Entry<String, ShopService.Client> cheapestShop = getCheapestShop(productName, productAmount);
        if (cheapestShop == null) {
            return false;
        }
        return buyProduct(productName, productAmount, cheapestShop);
    }

    private Map.Entry<String, ShopService.Client> getCheapestShop(String product, String refillAmount) {
        System.out.println(
                String.format(
        ...
        for (Map.Entry<String, ShopService.Client> currentShop : shops.entrySet()) {
            try {
                long currentPrice = currentShop.getValue().fetchProductPrice(product, refillAmount);
    ...
    private boolean buyProduct(String product, String amount, Map.Entry<String, ShopService.Client> shop) {
        System.out.println(
                String.format(
        ...
        try {
            if (shop != null) {
                if (shop.getValue().buyProduct(product, amount, deliveryAddress)) {
                    System.out.println(String.format(
                            "INFO : Buy product: %s, amount: %s Successful",
                            product, amount
                            )
                    );
                    Bill bill = new Bill(System.currentTimeMillis(), shop.getKey(), product, amount,
                            cheapestPrice);
